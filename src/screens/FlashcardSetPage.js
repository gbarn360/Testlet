...
export default function FlashcardSetPage() {
    ...
    const [flashcardCopy, setCopy] = useState([]);
        ...
        <div className="w-screen h-auto bg-slate-100 flex flex-col" onClick={(e) => { setClicked(e.target.value) }}>
            ...
            <div className="flex flex-col items-center">
                ...
                <div className="w-screen bg-slate-100" >
                    ...
                    <div className="flex flex-col items-center">
                        {flashcards.map((item) => <FlashcardTermDef term={item.term} definition={item.def} displayDeleteButton={displayDeleteButton} />)}
                    </div>
                </div>
            </div>
            <div className="flex flex-row mt-10">
                <div className="flex flex-row justify-start ml-40 space-x-10  w-3/4">
                    <button className="bg-blue-800 text-white pt-1 pb-1 pl-3 pr-3 rounded-md text-lg">Save</button>
                    <button onClick={() => { resetFlashcards() }} className="bg-slate-300 text-blue-800 pt-1 pb-1 pl-3 pr-3 rounded-md text-lg">revert</button>
                </div>
                <div className="w-1/4 ">
                    <button onClick={() => { setDisplayAddCard(true) }} className="border-2 border-blue-800 pt-1 pb-1 pl-3 pr-3 rounded-md text-lg">add card</button>
                </div>
            </div>
