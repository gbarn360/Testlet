...
export default function FlashcardTermDef({ term, definition, displayDeleteButton }) {
        ...
        <div className="flex flex-row justify-center w-screen">
            <div className="bg-white m-5 rounded-lg shadow-lg w-3/4  flex flex-row p-2 items-center" >
                <div className=" p-2 w-1/6 ">
                    <input value={updatedTerm} id="term" disabled={!readable} onChange={(e) => { setTerm(e.target.value) }} className={`w-full text-center bg-white ${selected}`} />
                </div>
                <div className=" border-l-2 p-2 h-auto w-5/6 ">
                    <input value={updatedDefinition} id="definition" disabled={!readable} readable onChange={(e) => { setDefinition(e.target.value) }} className={`w-full bg-white ${selected}`} />
                </div>
            ...
            <div className={`flex justify-center items-center ${displayRemoveButton}`} onClick={() => { alert("delete") }} >
